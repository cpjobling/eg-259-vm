...
You should read this in conjunction with the notes on [Regular Expressions](http://www.cpjobling.me/dokuwiki/eg-259:lecture7). Copy the text examples and regular expressions into am regular expression tool such as [RegexPal](http://regexpal.com).

Note this covers many more features of regular expressions than will be possible in one session. However, all the material is examinable.

Note in JavaScript a regular expression is entered between `/../`. In these examples, the slashes are omitted.
But for the first example, the equivalent JavaScript would be

    var matches = "cat".match(/cat/); // returns ["cat"]
    "Cat".matches(/cat/); // returns []

Many programming languages have a regular expressions library or regular expression literals built-in. What you learn here will be useful later when we look at PHP. But it is useful also for Java, Python, Perl, Ruby, MySQL Apache and even Unix (e.g. in editors emacs and vi, and the text processing tools grep, awk, egrep, and even to a limited extent, the shell).
...
will match both occurrences of *zz* in the word *pizzazz*. 
...
*Metacharacters* are special in regular expressions. The full list is
...
ASCII or ANSI control characters: `0xA9` (Hexadecimal A9) = `\xA9` as a regular expression.
